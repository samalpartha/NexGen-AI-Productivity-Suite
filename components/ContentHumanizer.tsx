...
import { Wand2, Loader2, Copy, ShieldCheck, ArrowRight } from 'lucide-react';
...
const ContentHumanizer: React.FC = () => {
  ...
  const handleHumanize = async () => {
    ...
    setLoading(true);
    try {
      const data = await humanizeContent(text);
      setResult(data);
    } catch (e) {
    ...
    <div className="h-full flex flex-col gap-6">
      <div className="bg-slate-800 p-4 rounded-xl border border-slate-700">
        ...
        <p className="text-slate-400 text-sm mt-1">
          Rewrite content to bypass AI detection and remove plagiarism flags by enhancing vocabulary and sentence structure.
        </p>
      </div>
      ...
      <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-6 min-h-0">
        ...
        <div className="flex flex-col gap-2">
          ...
          <textarea
            className="flex-1 w-full bg-slate-900 border border-slate-700 rounded-lg p-4 text-sm text-slate-200 focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none resize-none placeholder-slate-600 leading-relaxed"
        ...
        <div className="flex flex-col gap-2 relative">
          <label className="text-sm font-medium text-slate-300">Humanized Output</label>
          <div className="flex-1 w-full bg-slate-800 border border-slate-700 rounded-lg p-4 relative overflow-hidden group">
            {result ? (
